//Eulerian Trail in a given Eulerian or Semi-Eulerian Graph
#include <algorithm>
#include <iostream>
#include <list>
#include <string.h>
...
class Graph {
    ...
    //function to check if edge u-v is a valid next edge in Eulerian trail or circuit
    bool isValidNextEdge(int u, int v);
};
//The main function that print Eulerian Trail. It first finds an odd degree vertex (if there is any) and then
...
// function to check if edge u-v can be considered as next edge in Euler Tout
bool Graph::isValidNextEdge(int u, int v)
{
    // The edge u-v is valid in one of the following cases:
    //->If v is the only adjacent vertex of u
    int count = 0; 
    // To store count of adjacent vertices
    list<int>::iterator i;
    for (i = adj[u].begin(); i != adj[u].end(); ++i)
        if (*i != -1)
            count++;
    ...
    //->If there are multiple adjacents, then u-v is not a
...
//function removes edge u-v from graph.It removes the edge by replacing adjacent vertex value with -1.
